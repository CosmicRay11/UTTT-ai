...
class GUI():
    ...
    def __init__(self, master, board):
        ...
        self.label = tk.Label(master, text="Ultimate Tic Tac Toe")
        self.label.pack()

        self.next_player_var = tk.StringVar()
        self.next_player_var.set(board.next_player)
        
        self.next_player_label = tk.Label(master, textvariable = self.next_player_var)
        self.next_player_label.pack()


        self.grid = BoardGUI(master, board, self.update)
        self.grid.pack()

    def update(self):
        self.next_player_var.set(self.board.next_player)
...
class BoardGUI(tk.Frame):
            ...
            for y in range(3):
                ...
                for i in range(3):
                    for j in range(3):
                        ...
                        button.grid(row=i, column=j, ipadx = 20, ipady = 20)
    ...
    def update(self):
        ...
        if board.game_state() == "D":
            print("Draw")

            ### etc.. further things saying you won, etc....
